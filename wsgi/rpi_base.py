...
class RpiObject():
    """
    An abstract base class for all RPI components (i.e., services and
    platforms).
    ...
    (i.e., xmlrpc), and then format the data according to the RPI API
    ...
    def _connectRoutes(self, d):
        ...
        d.connect(self.__class__.__name__ + '-info', '/%s/%s/info' % (self._getUrlBase(), self.t), controller = self, action = 'info')
        d.connect(self.__class__.__name__ + '-stats', '/%s/%s/stats' % (self._getUrlBase(), self.t), controller = self, action = 'stats')
        d.connect(self.__class__.__name__ + '-doc', '/%s/%s/doc' % (self._getUrlBase(), self.t), controller = self, action = 'doc')
        d.connect(self.__class__.__name__ + '-releasenotes', '/%s/%s/releasenotes' % (self._getUrlBase(), self.t), controller = self, action = 'releasenotes')
        d.connect(self.__class__.__name__ + '-support', '/%s/%s/support' % (self._getUrlBase(), self.t), controller = self, action = 'support')
        d.connect(self.__class__.__name__ + '-source', '/%s/%s/source' % (self._getUrlBase(), self.t), controller = self, action = 'source')
        d.connect(self.__class__.__name__ + '-tryme', '/%s/%s/tryme' % (self._getUrlBase(), self.t), controller = self, action = 'tryme')
        d.connect(self.__class__.__name__ + '-licence', '/%s/%s/licence' % (self._getUrlBase(), self.t), controller = self, action = 'licence')
        d.connect(self.__class__.__name__ + '-provenance', '/%s/%s/provenance' % (self._getUrlBase(), self.t), controller = self, action = 'provenance')
...
class RpiService(RpiObject):
    def __init__(self, d):
        RpiObject.__init__(self, d)
        self.t = 'service'




class RpiPlatform(RpiObject):
    def __init__(self, d):
        RpiObject.__init__(self, d)
        self.t = 'platform'

    def _connectRoutes(self, d):
        # Call the parent's first:
        RpiObject._connectRoutes(self, d)
        # Now add platform-specific routes:
        d.connect(self.__class__.__name__ + '-factsheet', '/%s/%s/factsheet' % (self._getUrlBase(), self.t), controller = self, action = 'factsheet')
